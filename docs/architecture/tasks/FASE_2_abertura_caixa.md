...
## Status codex-guto - 2026-06-04

- [x] `OpeningBalanceMigrationService` criado com evento canonico `INVEST-OPENING-2026-01-01`.
- [x] Migracao idempotente: segunda execucao reaproveita perna existente e bloqueia divergencia acima de R$ 0,01.
- [x] `CashBalanceService` criado separando `settledCash`, `inTransit` e `cashWithTransit`.
- [x] `PatrimonyMtmDailyEngine` deixou de usar `cashIncludingTransit` como caixa base e nao soma transito duas vezes.
- [x] `DailyPatrimonyPoint` e `PatrimonyDailyStore` aceitam `settledCash` e `cashInTransit`, mantendo compatibilidade com campos legados.
- [x] Migration `37_invest_portfolio_daily_cash_components.sql` criada.
- [x] Endpoint `POST /api/invest/reconcile/migrate-opening-balance` adicionado.
- [x] `HoldingPurgeKeepOpeningService` preserva tambem o evento `INVEST-OPENING-2026-01-01`.
- [x] `npm run build` passou.
- [x] Testes focados passaram: OpeningBalanceMigrationService, PatrimonyMtmDailyEngine, PatrimonyDailyStore e HoldingPurgeKeepOpeningService.
- [ ] `npm test` completo rodou; 95/98 suites passaram. Falhas remanescentes fora deste pacote:
  - `tests/unit/invest/patrimonyAnchors.test.ts`: valores esperados de ancoras divergentes.
  - `tests/unit/controllers/InvestController.errors.test.ts`: Jest nao transforma `pdfjs-dist/legacy/build/pdf.mjs`.
  - `tests/unit/api/InvestApiRoutes.test.ts`: mesmo problema de `pdfjs-dist` ESM.
