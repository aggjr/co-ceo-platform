...
## Status codex-guto - 2026-06-03

- [x] `InventoryLedger.recordMovement` bloqueia gravacao sem `businessEventId`.
- [x] `FinancialLedger.record` bloqueia gravacao sem `businessEventId`.
- [x] Tipos dos ledgers exigem `businessEventId: string` em writes.
- [x] `InvestOperations.recordOperation` resolve/cria evento antes de gravar pernas.
- [x] Aberturas avulsas de posicao/caixa criam ou recebem evento.
- [x] `ReconciliationAuditService` reporta pernas orfas como `critical`.
- [x] `npm run build` passou.
- [x] Testes focados passaram: valuation, InventoryLedger status transitions e ReconciliationAuditService.
- [ ] `npm test` completo ainda nao passa por falhas remanescentes fora deste pacote:
...
- [x] Schema do banco nao foi alterado; `NOT NULL` fica para fase futura.
