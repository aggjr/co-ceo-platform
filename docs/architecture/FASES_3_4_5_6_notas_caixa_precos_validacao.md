...
- [ ] `BtgBrokerageNoteLedgerTranslator` nunca cria caixa `cleared` na nota
- [ ] Nota sempre cria perna `pending` (expectativa D+2)
- [ ] `LiqBolsaSettlementService` implementado com subset sum determinístico
- [ ] `BtgExtractLineParser` usa `LiqBolsaSettlementService` para `LIQ BOLSA`
- [ ] LIQ BOLSA não casado registra pendência crítica e não grava caixa
- [ ] Zero ocorrências de `AJUSTE DE DIVERGENCIA` criando dados artificiais
...
- [ ] `PatrimonyMtmDailyEngine` usa `CashBalanceService` sem dupla contagem
- [ ] `DailyPatrimonyPoint` tem `settledCash` e `cashInTransit` separados
- [ ] `PatrimonyDailyRebuildService.rebuild` respeita `lastTrustedDate`
- [ ] Migration 33 criada para as novas colunas
- [ ] `npm run build` sem erros / `npm test` sem regressões
...
- [ ] `computeThreePricesByUnderlying` aceita `asOfDate` e filtra eventos corretamente
- [ ] Zero usos de `ThreePricesValuation` no fluxo INVEST (endpoints, materialização, patrimônio)
- [ ] `ThreePricesValuation` marcado como `@deprecated`
- [ ] `MarketQuoteRepository` nunca usa preço atual como histórico
- [ ] `PatrimonyDailyRecorder.recordDay` bloqueia se ação sem cotação histórica
- [ ] `npm run build` sem erros / `npm test` sem regressões
...
- [ ] 3 endpoints de diagnóstico implementados e retornando dados
- [ ] As 10 perguntas do critério final têm resposta via API
