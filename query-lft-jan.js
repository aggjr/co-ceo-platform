...
async function run() {
  ...
  const [rows] = await c.query("SELECT e.transaction_date, a.asset_ticker, e.transaction_type, e.quantity, e.unit_price, e.total_gross_value, e.notes FROM invest_ledger_entries e JOIN invest_assets a ON e.asset_id = a.id WHERE e.transaction_date >= '2026-01-01' AND e.transaction_date <= '2026-01-10' AND a.asset_ticker LIKE '%LFT%' ORDER BY e.transaction_date ASC");
  console.table(rows);
  c.end();
}
