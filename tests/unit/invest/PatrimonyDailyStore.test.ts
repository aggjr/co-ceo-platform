...
describe('PatrimonyDailyStore invest_position_daily', () => {
  ...
  it('substitui integralmente o detalhe diario ao recalcular a mesma data', async () => {
    const gateway = new InMemoryGateway();
    const store = new PatrimonyDailyStore(gateway as any);
    const basePoint: DailyPatrimonyPoint = {
      date: '2026-04-17',
      patrimonyGross: 1_000,
      pendingSettlements: 0,
      scheduledCashPending: 0,
    ...
    expect(rows.map((r) => r.ticker)).toEqual(['CAIXA-BRL', 'CAIXA-TRANSIT', 'WEGE3']);
    expect(rows.some((r) => r.ticker === 'PRIO3')).toBe(false);
  });

  it('preserva a foto anterior quando o recálculo do detalhe falha', async () => {
    const gateway = new InMemoryGateway();
    const store = new PatrimonyDailyStore(gateway as any);
    const validPoint: DailyPatrimonyPoint = {
      date: '2026-04-17',
      patrimonyGross: 1_000,
      pendingSettlements: 0,
      scheduledCashPending: 0,
    ...
    const invalidPoint: DailyPatrimonyPoint = {
      ...validPoint,
      patrimony: 1_500,
      positionsValue: 1_000,
    };
    await expect(
      store.upsertPortfolioDay(ctx, {
        snapshotDate: '2026-04-17',
        point: invalidPoint,
        patrimonyGross: 1_500,
        fixedIncomeTotal: 0,
        externalFlow: 0,
        dailyReturnTwr: null,
        cumulativeTwr: null,
    ...
    expect(rows.map((r) => r.ticker)).toEqual(['CAIXA-BRL', 'CAIXA-TRANSIT', 'PRIO3']);
    expect(rows.some((r) => r.ticker === 'WEGE3')).toBe(false);
  });
  ...
  it('bloqueia fechamento diario quando o detalhe nao explica o agregado', async () => {
    ...
    expect(gateway.dump('invest_position_daily')).toHaveLength(0);
    expect(gateway.dump('invest_portfolio_daily')).toHaveLength(0);
  });
});
