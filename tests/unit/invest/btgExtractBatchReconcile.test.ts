...
describe('btgExtractBatchReconcile', () => {
  ...
  it('cadeia: saldo inicial bate com final do mes anterior no lote', () => {
    const feb = {
      path: '2026-02.pdf',
      fileName: '2026-02.pdf',
  ...
  it('cadeia quebrada torna o batch inseguro mesmo com delta nulo', () => {
    expect(
      batchChainIntact([
        { parseOk: true, openingChainOk: true },
        { parseOk: true, openingChainOk: false },
      ])
    ).toBe(false);
  });

  it('ordena extratos por mes', () => {
